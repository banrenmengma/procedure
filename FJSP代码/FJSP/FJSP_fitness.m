function [T,M,trace1,trace2,trace3,fitness]=FJSP_fitness(alldata,TaskAmo,SubtaskAmo,SubtaskNum,P,M,W)
...
for i=2:SubtaskAmo
    ...
    elseif (ismember(M(i),M_set))&&(ismember(P(i),P_set))
        ...
        temp10=find(temp8==temp9);
        %         if M(i)~=M(temp6(temp7-1))
        T(M(i),temp2)=max(T(M(temp6(temp7-1)),temp10)+W(temp6(temp7-1)),T(M_set(i),temp2-1)+W(temp1(temp2-1)));
        trace1(M(i),temp2)=W(i);
        trace2(M(i),temp2)=P(i);
        trace3(M(i),temp2)=temp7;
        %         else
        %             T(M(i),temp2)=max(T(M(temp6(temp7-1)),temp9-1)+W(M(temp6(temp7-1))),T(M_set(i),temp2-1)+W(M_set(temp1(temp2-1))));
        %             trace(M(i),temp2)=W(i);
        %         end
        maxspan=max(maxspan,T(M(i),temp2)+W(i));
    end
