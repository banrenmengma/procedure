function [T,M,trace1,trace2,trace3,fitness]=FJSP_ADV_fitness3(P,M,W)
SubtaskAmo=numel(P);
...
%         trace3(M(i),temp2)=1;
        ...
        flag2=0;
        for j=1:temp2-1
            if (EndTime(M(i),j)-StartTime(M(i),j)>=W(i)) && (EndTime(M(i),j)-E(M(temp6(temp7-1)),temp10)>=W(i))
                flag2=1;
                ...
                if StartTime(M(i),j)<E(M(temp6(temp7-1)),temp10)
                    temp11=EndTime(M(i),j);
                    EndTime(M(i),j)=E(M(temp6(temp7-1)),temp10);
...
%                 end
        ...
        if ~flag2
            ...
            maxspan=max(maxspan,T(M(i),temp2)+W(i));
            StartTime(M(i),temp2)=E(M(i),temp2-1);
            EndTime(M(i),temp2)=StartTime(M(i),temp2)+max(0,E(M(temp6(temp7-1)),temp10)-E(M_set(i),temp2-1));
            trace1(M(i),temp2)=W(i);
            trace2(M(i),temp2)=P(i);
